import { tool } from '@langchain/core/tools';
import fs from 'node:fs/promises';
import path from 'node:path';
import {
    spawn
} from 'node:child_process';
import { z } from 'zod';
import { ca } from 'zod/v4/locales';

// 读取文件工具
const readFileTool = tool(
    async ({ filePath }) => {
        try {
            console.log(`读取文件:${filePath}`);
            const content = await fs.readFile(filePath, 'utf-8');
            return `文件内容:${content}`;
        } catch (error) {
            console.log(`读取文件失败:${error.message}`);
            return `读取文件失败:${error.message}`;
        }
    },
    {
        name: 'read_file',
        description: '读取文件内容',
        schema: z.object({
            filePath: z.string().describe('文件路径')
        })
    }
)

// 写入文件工具
const writeFileTool = tool(
    async ({ filePath, content }) => {
        try {
            // 目录
            const dir = path.dirname(filePath);
            // 创建目录
            // recursive:true 递归创建目录
            await fs.mkdir(dir, { recursive: true });
            await fs.writeFile(filePath, content, 'utf-8');
            console.log(`[工具调用] write_file("${filePath}","${content}")`);
            return `文件写入成功:${filePath}`;
        } catch (error) {
            console.log(`写入文件失败:${error.message}`);
            return `写入文件失败:${error.message}`;
        }
    },
    {
        name: 'write_file',
        description: '向指定文件路径写入文件内容，自动创建目录',
        schema: z.object({
            filePath: z.string().describe('文件路径'),
            content: z.string().describe('要写入的内容')
        })
    }
)

// 执行命令工具
const execCommandTool = tool(
    async ({ command, workingDirectory }) => {
        try {
            const cwd = workingDirectory || process.cwd();// 默认当前工作目录
            console.log(`[工具调用] exec_command("${command}","${cwd}")`);
            return new Promise((resolve, reject) => {
                const [cmd, ...args] = command.split(' ');
                const child = spawn(cmd, args, {
                    cwd,
                    stdio: 'inherit',
                    shell: true
                })
                let errorMsg = '';
                child.on('error', (error) => {
                    errorMsg = error.message;
                });
                child.on('close', (code) => {
                    if (code === 0) {
                        console.log('命令执行成功');
                        const cwdInfo = workingDirectory ? `命令在目录${workingDirectory}下执行成功` : ``;
                        resolve(`命令执行成功:${cwdInfo}`);
                    } else {
                        const errorInfo = errorMsg
                            ? `错误: ${errorMsg}`
                            : `命令执行失败，退出码: ${code}`;
                        reject(new Error(errorInfo));
                    }

                });
            });
        } catch (error) {
            throw new Error(`执行命令失败:${error.message}`);
        }
    },
    {
        name: 'exec_command',
        description: '执行命令行命令',
        schema: z.object({
            command: z.string().describe('要执行的命令'),
            workingDirectory: z.string().optional().describe('命令执行的工作目录，默认为当前目录')
        })
    }
)

// 列出目录工具
const listDirectoryTool = tool(
    async ({ directoryPath }) =>{
        try {
            // 读取目录内容
            const files = await fs.readdir(directoryPath);
            console.log(`[工具调用] list_directory("${directoryPath}")`);
            return `目录内容:\n${files.map(file=>`-${file}`).join('\n')}`;
        }catch (error) {
            console.log(`[工具调用] list_directory("${directoryPath}")失败:${error.message}`);
            return `列出目录失败:${error.message}`;
        }
    },
    {
        name:'list_directory',
        description:'列出指定目录下的所有文件和子目录',
        schema:z.object({
            directoryPath:z.string().describe('目录路径')
        })
    }
)

export {
    readFileTool,
    writeFileTool,
    execCommandTool,
    listDirectoryTool
}