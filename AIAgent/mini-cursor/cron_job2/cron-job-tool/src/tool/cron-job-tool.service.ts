...
export class CronJobToolService {
    ...
    constructor() {
        const cronJobArgsSchema = z.object({
            ...
            id: z.string().optional().describe('任务ID，(toogle时需要)'),
            enabled: z.boolean().optional().describe('是否启用，(toogle可选； 不传则自动取反)'),
            ...
            \n2) 必须是自然语言描述， 不能是工具调用或代码（例如不能写send_mail(...) / db_users_crud(...) / web_search(...)）。
            \n3) 不要擅自补全细节或改写脚本。    
            `),
            cron: z.string().optional().describe('Cron表达式(type=cron时需要， 例如 */5 * * * * *)'),
            everyMs: z.number().int().positive().optional()
            .describe('固定间隔豪秒 (type=every 时需要，例如 60000 表示每分钟执行一次)'),
